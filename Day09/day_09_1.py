...
def update_knot(head_pos, tail_pos):
    ...
    assert distance_squared in [0,1,2,4,5] , f"The {distance_squared=} between {head_pos=} and {tail_pos=} is something different than 0,1,2,4 or 5."

    # If squared disstance is 4 or 5 we, can update it, otherwise no need to update since the point are touching
    if distance_squared in [4,5]:
...
# Initializing the positions of head and tail
head_pos = [0,0]
tail_pos = [0,0]
...
for move in puzzle_input:
    ...
    for _ in range(int(move_amount)):
        # Update the head and the tail nodes
        head_pos = move_step(head_pos, curr_step)
...
print(f"Final result for part 1 is {final_result}")        
